...
## Autocorrelation
pdff(paste0('autocorr_',setlabel))
lags <- 0:50
tplot(x=lags*0.04,y=cbind(coda::autocorr(coda::as.mcmc(longrunData$nspikes),lags),coda::autocorr(coda::as.mcmc(longrunData$stimulus),lags)),ylim=c(min(0,cbind(coda::autocorr(coda::as.mcmc(longrunData$nspikes),lags),coda::autocorr(coda::as.mcmc(longrunData$stimulus),lags))),NA),xlab='time/s',ylab='autocorrelation',lwd=4)
...
pdff(paste0('MIhisto_',setlabel,'_equalstim__prior',T,'_start2'))
for(lsample in c(2^(5:13), nrow(longrunData)-1)){
    ...
    tplot(x=postMIDistr$breaks,y=postMIDistr$density,
          ...
          main=paste0(setlabel,'; ',lsample, ' observed bins; start observation at bin ', startbin, '; prior weight: ', T),
          cex.main=1.25)
