...
pdff(paste0('MIhisto_inequalstim_shuffled_prior',T,'_start2'))
for(lsample in c(2^(5:13), nrow(longrunData)-1)){
    if(startr==0){
        entseq <- foreach(i=2L:(nrow(longrunData)-lsample+2L), .combine=c, .inorder=TRUE)%dopar%{
            entropy(
                tabulate(longrunData[i-1L+(1L:lsample),stimulus_lag1]+1L, nbins=nStimuli)
            )
        }
    ...
    sampleFreqs <- t(sapply(stimuli, function(stim){
        tabulate(sampleData[stimulus_lag1==stim,nspikes]+1L, nbins=maxSpikes1)
    }))
