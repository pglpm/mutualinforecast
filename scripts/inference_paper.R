...
## Last-Updated: 2021-03-20T09:22:49+0100
################
## Script for:
...
sampleIndexFile  <- 'index_mat_40.csv'
chunk <- 1
maxSpikes <- 15
...
##
## plot prior samples
pdff(paste0('plots_samples',nSamples,'_weight',priorWeight))
##
...
#####################################################
    ...
    rep(-maxy/20,100),type='p',lty=1,lwd=3,pch=18,cex=1,col=myblue,xlim=c(minx,maxx),ylim=c(-maxy/20,maxy),xlab=paste0('sampled MI/bit'),ylab='',main=maintext)
...
priorSamplePairs <- lapply(1:nStimuli, function(x){
        t(rdirichlet(n=nDistrSamples, alpha=priorWeight*priorBaseDistr))})

matr <- foreach(i in 1:10, .combine=rbind)%do%{
    
}


freqs <- lapply(1:nStimuli, function(x){
    temp <- round(rdirichlet(n=7, alpha=rep(1,6))*100)
    colnames(temp) <- paste0('x',0:5)
    rownames(temp) <- paste0('sample',1:7)
    temp
    })
names(freqs) <- 0:1

tablefreqs <- rbind(
    as.data.table(cbind(data.table(context=rep(0,7)),freqs[[1]])),
    as.data.table(cbind(data.table(context=rep(1,7)),freqs[[2]]))
    )

ggplot() +
    for(i in 1:10){
        geom_line(data=data.table(x=0:maxSpikes,y=priorSamplePairs[[1]][,i]),aes(x=x,y=y),size=1,alpha=0.5)
            }
