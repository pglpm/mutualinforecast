...
## Last-Updated: 2021-08-03T12:32:32+0200
################
## Script for:
...
sampleData <- foreach(i=stimulusVals, .combine=rbind)%do%{
    ...
    ##
    ## Probability density
    dpostp <- nimbleFunction(
        run = function(x=double(1), shapegamma=double(0), rategamma=double(0), priorstrength=double(0), datafreqs=double(1), maxS=double(0), log=integer(0, default=0)){
            returnType(double(0))
            mean <- exp(x[1])
            lnf <- x[2:(maxS+2)]
            tx <- sum(lnf)
            f <- exp(lnf)/sum(exp(lnf))
            alphas <- priorstrength * geomd(x=mean, y=0:maxS) + datafreqs
            ##
            logp <- dgamma(x=mean, shape=shapegamma, rate=rategamma, log=1) + x[1] +
                ddirch(x=f, alpha=alphas, log=1) + sum(log(f)) - tx^2
            if(log) return(logp) else return(exp(logp))
        })
    rpostp <- nimbleFunction(
        run = function(n=integer(0), shapegamma=double(0), rategamma=double(0), priorstrength=double(0), datafreqs=double(1), maxS=double(0)){
            print('Error calling the r-function')
            return(c(Inf,Inf))
            returnType(double(1))
})
    assign('dpostp', dpostp, envir = .GlobalEnv)
    assign('rpostp', rpostp, envir = .GlobalEnv)
    ## Cdlogsmoothmean <- compileNimble(dlogsmoothmean)
##
code <- nimbleCode({
    for(i in 1:nStimuli){
        X[i, ] ~ dpostp(shapegamma=shapegammac, rategamma=rategammac, priorstrength=priorstrengthc, datafreqs=datafreqsc[i, ], maxS=maxS)
        means[i] <- exp(X[i,1])
        F[i, 1:maxS1] <- exp(X[i, 2:(maxS1+1)])/sum(exp(X[i, 2:(maxS1+1)]))
            }
    })
    ##
    constants <- list(priorstrengthc=priorstrength, shapegammac=shapegamma, rategammac=rategamma, nStimuli=nStimuli, maxS1=maxS1, maxS=maxS, chunk=chunk, datafreqsc=sampleFreqs)
    ##
    initX <- normalizerows(sampleFreqs+1)
    initmeans <- rep(priorMeanSpikes,nStimuli)
    inits <- list(X=cbind(log(initmeans),log(initX)-rowSums(log(initX))/maxS1))
##
modeldata <- list() #if(chunk>-1){list(datafreqs=sampleFreqs, size=rowSums(sampleFreqs))}else{list()}
##
    model <- nimbleModel(code=code, name='model', constants=constants, inits=inits, data=modeldata, dimensions=list(X=c(nStimuli,maxS1+1), F=c(nStimuli,maxS1), means=nStimuli), calculate=F)
...
for(i in 1:nStimuli){
    confmcmc$addSampler(target=paste0('X[',i,', ]'), type='AF_slice', control=list(sliceAdaptFactorMaxIter=20000, sliceAdaptFactorInterval=1000, sliceAdaptWidthMaxIter=1000, sliceMaxSteps=100, maxContractions=100))
    }
    ## for(i in 1:nStimuli){
    ...
    ## }
    confmcmc$setMonitors(c('means','F','logProb_X'))
    confmcmc
    ...
    llsamples <- mcsamples[,(maxS1*nStimuli)+(1:2),drop=F]
##
