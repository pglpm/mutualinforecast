...
%% Last-Updated: 2020-08-09T19:23:38+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\yffa}{f^{\alphaup}}
\newcommand*{\yffb}{f^{\betaup}}
\newcommand*{\yfa}{\bm{f}^{\alphaup}}
\newcommand*{\yfb}{\bm{f}^{\betaup}}
\newcommand*{\yFFa}{F^{\alphaup}}
\newcommand*{\yFFb}{F^{\betaup}}
\newcommand*{\yFa}{\bm{F}^{\alphaup}}
\newcommand*{\yFb}{\bm{F}^{\betaup}}
...
\begin{equation}
  ...
  \bm{x}/\bm{y} \defd (x_{1}/y_{1}, \dotsc, x_{10}/y_{10}) \ ,
  \qquad
  \vsum \,\bm{x} \ln\bm{y} \defd \tsum_{i} x_{i} \ln y_{i} \ ,
\end{equation}
%where $\lb \defd \log_{2}$  \parencite[convention][item 2-12.7]{iso2009b}
and let
\begin{equation}
  \label{eq:entropies}
  \hh(\bm{x}) \defd -\vsum\,\bm{x}\ln\bm{x}\ ,
  \qquad
  \dd(\bm{x} , \bm{y}) \defd \vsum\,\bm{x}\ln\bm{x}/\bm{y}
\end{equation}
be the Shannon entropy and relative entropy for distributions $\bm{x}$,
$\bm{y}$.


Let $\yfa \defd (\yffa_{1}, \dotsc, \yffa_{10})$ and $\yfb$ be the response
frequencies observed in the sample, conditional on the two kinds of
stimulus $\alphaup$ and $\betaup$. Let
$\yFa \defd (\yFFa_{1}, \dotsc, \yFFa_{10})$ and $\yFb$ be the long-run
...
\begin{equation}
  ...
  \begin{split}
  \ii(\yFa, \yFb) &\equiv
  \vsum\,\frac{1}{2}\yFa\ln\frac{2\yFa}{\yFa + \yFb} +
  \vsum\,\frac{1}{2}\yFb\ln\frac{2\yFb}{\yFa + \yFb}
  % \\[\jot]
...
\begin{equation}
  \label{eq:drawurn}
  \begin{split}
  \pf( \text{\small seq}^{\alphaup}, \text{\small seq}^{\betaup} \| \yFa,\yFb , A)
  &=
  \Biggl[\prod_{r=1}^{10}  \bigl(\yFFa_{r}\bigr)^{n\,\yffa_{r}} \Biggr] \times
  \Biggl[\prod_{r=1}^{10}  \bigl(\yFFb_{r}\bigr)^{n\,\yffb_{r}}\Biggr]
  \\[\jot]
&\equiv  \exp\bigl( n\vsum \yfa\ln\yFa + n\vsum \yfb\ln\yFb \bigr)
...
\begin{multline}
  ...
  \pf( \yFa, \yFb \|
  \text{\small seq}^{\alphaup}, \text{\small seq}^{\betaup}, A) 
  \\[\jot]
  \begin{aligned}
    &= \frac{\exp\bigl( n\vsum \yfa\ln\yFa + n\vsum \yfb\ln\yFb \bigr) \times
      \pf(\yFa,\yFb \| A)}{
      \int \exp\bigl( n\vsum \yfa\ln\yFa + n\vsum \yfb\ln\yFb \bigr) \times
  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb}
  \\[2\jot]
    &= \frac{\exp\bigl[ - n \, \dd(\yfa , \yFa) - n\, \dd(\yfb , \yFb)  \bigr]
      \times
      \pf(\yFa,\yFb \| A)}{
      \int \bigl[ - n \, \dd(\yfa , \yFa) - n \, \dd(\yfb , \yFb)  \bigr]
      \times  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb}
 \end{aligned}
\end{multline}
...
of guessing the long-run frequencies:
\begin{equation}
...
\end{equation}
that is, the sample frequencies are sufficient statistics.
...
\figs~\ref{fig:superdistributions}--\ref{fig:backward} do). This would lead
(for convexity reasons) to a broadening of the probability distribution for
$M$, indicating that we are indeed basing our inference on a reduced amount
of data. \mynote{Or?}
