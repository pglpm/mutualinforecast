...
%% Last-Updated: 2020-08-13T08:21:39+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{innote}
  ...
  represented by a vector $\bm{x} \defd
  \Bigl(\begin{smallmatrix}
    x_{1}\\x_{2}\\\dotso
  \end{smallmatrix}\Bigr)$; the four arithmetic operations act element-wise
  on vectors and covectors. The logarithm acts element-wise and maps
  a vector into a covector:
  $\ln\Bigl(\begin{smallmatrix} x_{1}\\x_{2}\\\dotso
  \end{smallmatrix}\Bigr) \defd \bigl(\ln x_{1}\ \ln x_{2}\ \dotso\bigr)$.
  Multiplication of a vector and a covector (in any order) yields a scalar.
\end{innote}

Label the possible responses by $r$. Let
$\yfa \defd (\yffa{1}, \dotsc, \yffa{r},\dotsc)$ and $\yfb$ be the
response frequencies observed in the sample, conditional on the two kinds
of stimulus $\alphaup$ and $\betaup$. Let
$\yFa \defd \bigl(\yFFa{r} \bigr)$ and $\yFb$ be the long-run
response frequencies, conditional on the two kinds of stimulus. If the two
kinds of stimulus are assumed to occur equally often in the sample and in
the long run, then the mutual information between the stimulus frequencies
and response long-run frequencies is given by the specific formula
\begin{equation}
...
value $M$ within $\di M$, given the observed conditional frequencies
...
superdistribution), has the following formula:
\begin{multline}
  \label{eq:full_inference}
  \pf( \ii \eq M \| \yfa, \yfb, A) \propto
  \int
  \delt\bigl[M - \ii(\yFa, \yFb) \bigr] \times{}\\[\jot]
  \exp\bigl[ - n \, \dd(\yfa , \yFa) - n \, \dd(\yfb , \yFb)  \bigr] \
...
\begin{multline}
  \label{eq:margin_I}
  \pf( \ii \eq M \| \yfa, \yfb, A) \\
  \begin{aligned}
&=  \int \pf( \ii \eq M \| \yFa, \yFb, \yfa, \yfb, A) \times
  \pf(\yFa,\yFb \| \yfa, \yfb, A)\,\di\yFa\,\di\yFb
  \\
   &=
  \int  \delt\bigl[M - \ii(\yFa, \yFb) \bigr] \times
  \pf(\yFa,\yFb \| \yfa, \yfb, A)\,\di\yFa\,\di\yFb
 \end{aligned}
\end{multline}
...
The probability of observing two sequences of responses, each of length
$n$, in which the responses appear with frequencies $\yfa$ and $\yfb$,
...
from two \enquote{urns} with known contents. We can rewrite it with
exponentials:
\begin{equation}
  \label{eq:drawurn}
  \begin{split}
...
\begin{multline}
  ...
  \begin{aligned}
    &= \frac{\exp\bigl( n\, \yfa\ln\yFa + n\, \yfb\ln\yFb \bigr) \times
      \pf(\yFa,\yFb \| A)}{
      \int \exp\bigl( n\, \yfa\ln\yFa + n\, \yfb\ln\yFb \bigr) \times
  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb}
  \\[2\jot]
    &= \frac{\exp\bigl[ - n \, \dd(\yfa , \yFa) - n\, \dd(\yfb , \yFb)  \bigr]
      \times
      \pf(\yFa,\yFb \| A)}{
      \int \exp\bigl[ - n \, \dd(\yfa , \yFa) - n \, \dd(\yfb , \yFb)  \bigr]
      \times  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb}
 \end{aligned}
\end{multline}
...
distribution for $M$, indicating that we are indeed basing our inference on
...
an intuitive way when our sample is large: for large sample size $n$ the
exponential term becomes a delta centred on the observed frequencies. The
long-run mutual information is therefore approximately equal to the sample
mutual information. \mynote{One can also do expansions in $n$ to obtain
  asymptotic corrections. The exponential term also shows where
  maximum-entropy methods come from.}
