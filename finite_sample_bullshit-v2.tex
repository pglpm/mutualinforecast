...
%% Last-Updated: 2020-09-28T14:07:44+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\section{}Motivation for mutual info and description of the problem


\section{}Explain (figures, graphs) that reasoning about different samples is
misleading.


\section{} Need to focus on long-run freqs -> they give us long-run MI. Need to
focus on sample freqs -> sample MI is 1. misleading, 2. gives less info.
Sufficient stats.


\section{} Claudia

Examples with equally likely long-run freqs – we can have very different
longrun MI leading to same sample MI. Introduced by a discrete example.
Multinomial probabilities


\section{} from the guess of the long-run freqs we guess the long-run MI -> distributions.


\section{} Claudia.

Weights (prior distribution) are needed besides likelihood – biological
reasons or average reasons. Still from discrete example. Unavoidability of
choice.


\section{} Soledad.

Motivation of different choices of weights/distribution/prior

\section{} Soledad. Generalization to continuum -> Bayes's theorem
(commented)


\section{}Dependence on number of samples. Why likelihood can become enough and why
sample MI is good guess. Laplace approx.


\section{}Why a full distribution is better than a point estimate (+ variance).
Possible uses in neuroscience of this distribution.


\section{}Discussion of bias: becomes irrelevant
...
\end{equation}
%where $\lb \defd \log_{2}$  \parencite[convention][item 2-12.7]{iso2009b}
and let
\begin{equation}
  \label{eq:entropies}
...
\begin{equation}
 ...
 \frac{1}{2}\yFa\ln\frac{2\yFa}{\yFa + \yFb} +
  ...
  % \\[\jot]
  % &\equiv \hh\biggl(\frac{\yFa + \yFb}{2}\biggr)
...
The probability density that the long-run mutual information has
value $\ii=\yIv$ within $\di \yIv$, given the observed conditional frequencies
$\yfa$, $\yfb$ in a sample of $n$ each, and given the superdistribution
$\pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb$ for the long-run conditional
frequencies ($A$ denotes the specific assumptions behind the
superdistribution), has the following expression:
\begin{multline}
  \label{eq:full_inference}
  \pf( \yIv \| \yfa, \yfb, A) \propto
  \iint\!
  \delt\bigl[\yIv - \ii(\yFa, \yFb) \bigr] \times{}\\[\jot]
  \exp\bigl[ - n \, \dd(\yfa , \yFa) - n \, \dd(\yfb , \yFb)  \bigr] \times
  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb \ .
\end{multline}
...
\begin{multline}
  ...
  \pf( \yIv \| \yfa, \yfb, A) \\
  \begin{aligned}
&=  \iint\! \pf( \yIv \| \yFa, \yFb, \yfa, \yfb, A) \times
  \pf(\yFa,\yFb \| \yfa, \yfb, A)\,\di\yFa\,\di\yFb
  \\
   &=
  \iint\!  \delt\bigl[\yIv - \ii(\yFa, \yFb) \bigr] \times
  \pf(\yFa,\yFb \| \yfa, \yfb, A)\,\di\yFa\,\di\yFb
 \end{aligned}
\end{multline}
where the second equality comes from the fact that the mutual information
is completely determined by the long-run conditional frequencies.

The probability of observing a sequence of $n$ responses for each 
stimulus, with the responses appearing with frequencies $\yfa$ and $\yfb$,
given the long-run frequencies $\yFa$, $\yFb$, can be calculated by simple
combinatorics. It's essentially the formula for drawing with replacement
from two \enquote{urns} with known contents
\citep{definetti1938}[\sect~4.6]{bernardoetal1994_r2000}. We can rewrite it
with exponentials:
\begin{equation}
  \label{eq:drawurn}
  \begin{split}
  \pf( \text{\small seq}_{\alphaup}, \text{\small seq}_{\betaup} \| \yFa,\yFb , A)
  &=
  \biggl[\prod_{r}  \bigl(\yFFa{r}\bigr)^{n\,\yffa{r}} \biggr] \times
  \biggl[\prod_{r}  \bigl(\yFFb{r}\bigr)^{n\,\yffb{r}}\biggr]
  \\[\jot]
&\equiv  \exp\bigl( n\, \yfa\ln\yFa + n\, \yfb\ln\yFb \bigr) \ .
 % \times \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb
\end{split}
\end{equation}

Using the rule of conditional probability we obtain the density in
$\di\yFa\,\di\yFb$:
\begin{multline}
  \label{eq:definetti_bayes}
  \pf( \yFa, \yFb \|
  \text{\small seq}_{\alphaup}, \text{\small seq}_{\betaup}, A) 
  \\[\jot]
  \begin{aligned}
    &= \frac{\exp\bigl( n\, \yfa\ln\yFa + n\, \yfb\ln\yFb \bigr) \times
      \pf(\yFa,\yFb \| A)}{
      \iint\! \exp\bigl( n\, \yfa\ln\yFa + n\, \yfb\ln\yFb \bigr) \times
  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb}
  \\[2\jot]
    &= \frac{\exp\bigl[ - n \, \dd(\yfa , \yFa) - n\, \dd(\yfb , \yFb)  \bigr]
      \times
      \pf(\yFa,\yFb \| A)}{
      \iint\! \exp\bigl[ - n \, \dd(\yfa , \yFa) - n \, \dd(\yfb , \yFb)  \bigr]
      \times  \pf(\yFa,\yFb \| A)\,\di\yFa\,\di\yFb}
 \end{aligned}
\end{multline}
where the second equality is obtained multiplying numerator and denominator
by $\exp[n \hh(\yfa) + n \hh(\yfb)]$.

The formula above also shows that the knowledge of the sample frequencies
is equivalent to the knowledge of the specific sequences, for the purpose
of guessing the long-run frequencies. That is, the sample frequencies are
sufficient statistics:
\begin{equation}
  \label{eq:sufficient_freqs}
  \pf( \yFa, \yFb \|
  \text{\small seq}_{\alphaup}, \text{\small seq}_{\betaup}, A) =
  \pf( \yFa, \yFb \| \yfa, \yfb, A) \ .
\end{equation}

\medskip

Finally substituting formula~\eqref{eq:definetti_bayes}
into~\eqref{eq:margin_I} we obtain~\eqref{eq:full_inference}.

\medskip

If we only knew the sample mutual-information, but not the sample
frequencies, it would be necessary to marginalize the
probability~\eqref{eq:full_inference} over all possible values of the
sample frequencies that have the observed mutual information (this is what
\figs~\ref{fig:superdistributions}--\ref{fig:backward} do). This would
lead, for convexity reasons, to a broadening of the probability
distribution for $\yIv$, indicating that we are indeed basing our inference on
a reduced amount of data. In fact the sample mutual information is not a
...
\begin{center}%[h!]%{l}{0.75\linewidth} % with wrapfigure
...
choice depends on the context: the specific experiment, brain region, cell
type, and all other biological details. We imagine three example cases, each
of which could be appropriate in a different context.

First. We consider all long-run response-frequency distributions, for each
stimulus, to be equally probable in frequency space: peaked or flat, broad
or narrow, and so on (this belief is represented by a \enquote{flat}
Dirichlet superdistribution with unit parameters). Our resulting guess about the
long-run mutual information is given by the probability distribution at the
top of \fig~\ref{fig:example_result}. There is a 95\% probability that the
long-run mutual info is between 0.093 and 0.371~bit, and a 50-50\%
probability that it is lower or higher than 0.214~bit.
\begin{figure}[p]%{l}{0.75\linewidth} % with wrapfigure
...
belief is represented here by a Dirichlet superdistribution with parameters less
than unity, very close to a Jeffreys prior). Our resulting guess about the
long-run mutual information is given by the probability distribution in the
middle of \fig~\ref{fig:example_result}. There is a 95\% probability that
the long-run mutual info is between 0.245 and 0.591~bit, and a 50-50\%
...
responses, rather than to their frequencies (this belief is represented
here by a Dirichlet superdistribution with parameters larger than unity).
Our resulting guess about the long-run mutual information is given by the
probability distribution at the bottom of \fig~\ref{fig:example_result}.
There is a 95\% probability that the long-run mutual info is between 0.0103
and 0.0676~bit, and a 50-50\% probability that it is lower or higher than
0.0309~bit.
...
those other experiments, using a hierarchic model.
